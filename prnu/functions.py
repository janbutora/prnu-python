...
def noise_extract(im: np.ndarray, levels: int = 4, sigma: float = 5) -> np.ndarray:
    ...
    :param im: grayscale or color image
    :param levels: number of wavelet decomposition levels
    :param sigma: estimated noise power
    :return: noise residual
    """
...
def noise_extract_compact(args):
    ...
    """
    im, levels, sigma, dimples, watermarked, ws = args
    if dimples:
        im = remove_dimples(im)
    w = noise_extract(im, levels, sigma)
    if watermarked:
        w = cancel_watermark(w, ws)
    return (w * im / 255.).astype(np.float32)


def extract_multiple_aligned(imgs: list, levels: int = 4, sigma: float = 5, dimples=False,
                             watermarked=False, ws=1, processes: int = None,
    ...
    if processes is None or processes > 1:
        args_list = []
        for im in imgs:
            args_list += [(im, levels, sigma, dimples, watermarked, ws)]
        ...
        for im in tqdm(imgs, disable=tqdm_str is None, desc=tqdm_str, dynamic_ncols=True):
            if dimples:
                im = remove_dimples(im)
            RPsum += noise_extract_compact((im, levels, sigma, False, watermarked, ws))
...
def pce(cc: np.ndarray, neigh_radius: int = 2, shifts: bool = False) -> dict:
    """
    ...
    if shifts:
        ...
        max_y, max_x = np.array(cc.shape)-1

    peak_height = cc[max_y, max_x]

    cc_nopeaks = cc.copy()
    cc_nopeaks[np.maximum(0,max_y - neigh_radius):max_y + neigh_radius, np.maximum(0,max_x - neigh_radius):max_x + neigh_radius] = 0

    pce_energy = np.mean(cc_nopeaks.flatten() ** 2)*cc.size/(cc.size-cc[np.maximum(0,max_y - neigh_radius):max_y + neigh_radius, np.maximum(0,max_x - neigh_radius):max_x + neigh_radius].size)
...
"""
Functions for removing various biases
"""
def remove_dimples(im: np.ndarray) -> np.ndarray:
    """
    Remove the JPEG dimples from a color pixel image
    :param im: image with potential JPEG dimples
    :return: image without the JPEG dimples
    """
    im = np.float32(im)
    im_arr = view_as_blocks(im, (8,8,3))
    dimples = np.mean(im_arr, axis=(0,1,2))
    dimples -= np.mean(dimples, axis=(0,1))
    im_arr -= dimples
    return im

def estimate_watermark(res: np.ndarray, ws: int) -> np.ndarray:
    """
    Estimate a potential watermark from an image residual
    :param res: color image residual
    :param ws: size of a potential (square) watermark
    :return: the residual without a square watermark of size (ws,ws)
    """
    watermark = np.mean(view_as_blocks(rgb2gray(res)[:res.shape[0]//ws*ws, :res.shape[1]//ws*ws], (ws,ws)), axis=(0,1))
    return watermark

def cancel_watermark(res: np.ndarray, ws: int) -> np.ndarray:
    """
    Cancel a potential watermark from an image residual
    :param res: image residual
    :param ws: size of a potential (square) watermark
    :return: the residual without a square watermark of size (ws,ws).
    The resulting residual is cropped to multiples of (ws,ws)
    """
    res = np.float32(np.array(res))
    h,w = np.array(res.shape[:2]) // ws * ws
    c = res.shape[2]
    res = res[:h,:w,:] # only use multiples of ws

    watermark = estimate_watermark(res, ws)

    # Modified Gram-Schmidt process
    res_arr = np.squeeze(view_as_blocks(res, (ws,ws,1))) # new shape (h,w,c,WS,WS)
    projection = np.einsum('abcde,de->abc', res_arr,watermark).reshape(h//ws,w//ws,c,1,1)/np.linalg.norm(watermark)**2
    projection = np.clip(projection,0, projection.max())
    res_arr = res_arr - projection*watermark
    return reshape_color_view_to_original(res_arr,res)

def reshape_color_view_to_original(arr, orig):
    """
    Reshapes a view into color array into its original size
    :param arr: array of size (bh,bw,c,h,w)
    :param orig: array of size (bh*h,bw*w,,c)
    :return: the input array arr in the same shape as orig
    """
    return np.transpose(arr, [0,3,1,4,2]).reshape(orig.shape)
